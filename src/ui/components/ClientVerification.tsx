...
export const ClientVerification: React.FC<ClientVerificationProps> = ({ onReport }) => {
  ...
  useEffect(() => {
    ...
    apiService.validateQR(token)
      ...
      .catch(() => {
        setError('QR Expired or Invalid');
        setLoading(false);
      });
  ...
  if (loading) return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-slate-50">
      ...
      <p className="text-slate-400 font-black text-[10px] tracking-widest uppercase">Validating Digital Identity...</p>
    </div>
  );
  ...
  if (error || !techData) return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-red-50 p-8 text-center">
      ...
      <h1 className="text-3xl font-black text-red-800 mb-2 uppercase tracking-tighter">SECURITY ALERT</h1>
      <p className="text-red-600 font-bold text-lg mb-8 uppercase">INVALID DOCUMENT</p>
      <div className="bg-white p-6 rounded-xl shadow-lg border border-red-200 max-w-sm mb-10">
        <p className="text-slate-600 font-medium italic">This QR code has expired or does not belong to authorized Fibex Telecom personnel. Please contact security immediately.</p>
      </div>
      <button onClick={() => window.location.href = 'tel:911'} className="bg-red-600 text-white w-full max-w-xs py-4 rounded-xl font-black shadow-xl active:scale-95 transition-all">CALL AUTHORITIES</button>
    </div>
  );
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col items-center pb-12 font-sans overflow-x-hidden">
      <header className="w-full bg-slate-900 h-16 flex items-center justify-between px-6 shadow-xl sticky top-0 z-50">
        <div className="flex items-center space-x-2">
           ...
           <span className="text-white font-black text-[10px] tracking-wider uppercase">Fibex Personnel</span>
        </div>
        <div className="bg-blue-600 px-3 py-1 rounded-full flex items-center space-x-2 border border-blue-400">
           <div className="w-1.5 h-1.5 bg-white rounded-full animate-pulse"></div>
           <span className="text-white font-black text-[9px] uppercase tracking-tighter">Active ID</span>
        </div>
      </header>

      <div className="w-full bg-green-500 py-3 flex items-center justify-center space-x-3 shadow-inner">
         <span className="text-[10px] font-black text-white uppercase tracking-[0.3em]">VALIDATION SUCCESSFUL • AUTHORIZED PERSONNEL</span>
      </div>
      ...
      <main className="w-full max-w-sm px-6 mt-8 relative">
        ...
        <div className="bg-white rounded-3xl shadow-2xl overflow-hidden border border-slate-200 relative">
          <div className="h-32 bg-slate-900 relative flex items-end justify-center">
             <div className="absolute inset-0 opacity-10 bg-[url('https://images.unsplash.com/photo-1557682250-33bd709cbe85?auto=format&fit=crop&w=800&q=80')] bg-cover"></div>
             ...
             <div className="w-40 h-40 rounded-3xl border-[8px] border-white shadow-2xl bg-white overflow-hidden transform translate-y-12 z-10 transition-transform hover:scale-105 duration-500">
                <img 
                  src={techData.photoUrl || `https://i.pravatar.cc/300?u=${techData.documentId}`} 
                  alt={techData.name} 
          ...
          <div className="pt-20 pb-8 px-8 text-center">
             ...
             <div className="flex items-center justify-center space-x-2 mb-6">
                <span className="bg-blue-50 text-blue-700 px-3 py-1 rounded-full text-[9px] font-black uppercase border border-blue-100 italic">{techData.role || 'Specialist III'}</span>
                <span className="w-1 h-1 bg-slate-300 rounded-full"></span>
                <span className="text-slate-500 font-bold text-[10px] uppercase tracking-widest">{techData.company || 'Fibex Services'}</span>
             </div>

             <div className="grid grid-cols-4 gap-2 mb-8 bg-slate-50 p-4 rounded-2xl border border-slate-100 shadow-inner">
                {['BASIC', 'CORE', 'INTEGRAL', 'PREMIUM'].map((lvl) => {
                  const isActive = techData.level?.toUpperCase() === lvl;
                  return (
             ...
             <div className="space-y-3">
                <div className="flex justify-between items-center bg-slate-50 p-3 rounded-lg border border-slate-100">
                   <span className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">Document ID</span>
                   <span className="text-xs font-black text-slate-900">{techData.documentId}</span>
                </div>
                <div className="flex justify-between items-center bg-slate-50 p-3 rounded-lg border border-slate-100">
                   <span className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">Work Status</span>
                   <div className="flex items-center space-x-2">
                ...
                <div className="flex justify-between items-center bg-slate-50 p-3 rounded-lg border border-slate-100">
                   <span className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">Jurisdiction</span>
        ...
        <div className="mt-8 space-y-4">
           ...
           >
              ...
              <span className="text-[10px] font-black uppercase tracking-widest">Report Security Incident</span>
           </button>
           
           <div className="text-center opacity-30">
              <p className="text-[8px] font-black text-slate-900 uppercase tracking-[0.4em]">Fibex Security Protocol v5.0 Master</p>
           </div>
        </div>
      </main>

      <footer className="mt-auto px-10 text-center">
         <p className="text-[10px] text-slate-300 font-bold uppercase tracking-widest">Protecting Fibex Telecom Critical Infrastructure</p>
      </footer>
    </div>
  );
};
