...
export const ClientVerification: React.FC<ClientVerificationProps> = ({ onReport }) => {
  ...
  if (error || !techData) return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-red-50 p-8 text-center">
      ...
      <p className="text-red-600 font-bold text-lg mb-8 uppercase">DOCUMENTO INVÁLIDO</p>
      <div className="bg-white p-6 rounded-xl shadow-lg border border-red-200 max-w-sm mb-10">
        <p className="text-slate-600 font-medium italic">Este código QR ha expirado o no pertenece al personal autorizado de Fibex Telecom. Por favor, contacte a seguridad de inmediato.</p>
      </div>
      <button onClick={() => window.location.href = 'tel:911'} className="bg-red-600 text-white w-full max-w-xs py-4 rounded-xl font-black shadow-xl active:scale-95 transition-all uppercase">Llamar a las Autoridades</button>
    </div>
  );
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col items-center pb-12 font-sans overflow-x-hidden">
      <header className="w-full bg-slate-900 h-16 flex items-center justify-between px-6 shadow-xl sticky top-0 z-50">
        ...
        <div className="bg-blue-600 px-3 py-1 rounded-full flex items-center space-x-2 border border-blue-400">
           ...
           <span className="text-white font-black text-[9px] uppercase tracking-tighter">ID Activo</span>
        </div>
      </header>

      <div className="w-full bg-green-500 py-3 flex items-center justify-center space-x-3 shadow-inner">
         <span className="text-[10px] font-black text-white uppercase tracking-[0.3em]">VALIDACIÓN EXITOSA • PERSONAL AUTORIZADO</span>
      </div>
      ...
      <main className="w-full max-w-sm px-6 mt-8 relative">
        ...
        <div className="bg-white rounded-3xl shadow-2xl overflow-hidden border border-slate-200 relative">
          ...
          <div className="pt-20 pb-8 px-8 text-center">
             ...
             <div className="flex items-center justify-center space-x-2 mb-6">
                ...
                <span className="text-slate-500 font-bold text-[10px] uppercase tracking-widest">{techData.company || 'Servicios Fibex'}</span>
             </div>
             ...
             <div className="space-y-3">
                <div className="flex justify-between items-center bg-slate-50 p-3 rounded-lg border border-slate-100">
                   <span className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">Cédula / ID</span>
        ...
        <div className="mt-8 space-y-4">
           ...
           <div className="text-center opacity-30">
              <p className="text-[8px] font-black text-slate-900 uppercase tracking-[0.4em]">Protocolo de Seguridad Fibex v5.0 Master</p>
           </div>
        </div>
      </main>

      <footer className="mt-auto px-10 text-center">
         <p className="text-[10px] text-slate-300 font-bold uppercase tracking-widest italic py-8">Protegiendo la Infraestructura Crítica de Fibex Telecom</p>
      </footer>
    </div>
  );
};
