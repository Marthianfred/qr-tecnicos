  ...
  return (
    <div className="min-h-screen bg-slate-900 text-white font-sans">
      <header className="p-8 border-b border-white/5 flex justify-between items-center">
        <div>
          <h1 className="text-2xl font-black uppercase tracking-tighter italic">Consola de Agente de Campo</h1>
          <p className="text-[10px] font-bold text-blue-400 uppercase tracking-widest">Protocolo Operativo Fibex v5.0 Master</p>
        </div>
        <button 
        ...
        >
          {qrVisible ? 'Cerrar Identidad' : 'Generar QR de Verificación'}
        </button>
      </header>
      ...
      <main className="p-10 max-w-5xl mx-auto space-y-12">
        {qrVisible && (
          <div className="bg-white rounded-[2.5rem] p-12 text-slate-900 flex flex-col items-center shadow-2xl animate-in zoom-in-95 duration-300">
            <h2 className="text-xl font-black uppercase tracking-tighter mb-2 italic">Identificación Oficial</h2>
            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-10">Escanee para verificación de seguridad del cliente</p>
            ...
            <div className="mt-10 text-center space-y-2">
               <div className="flex items-center space-x-2 justify-center">
                  ...
                  <p className="text-xs font-black uppercase tracking-widest">Token de Autorización en Vivo</p>
               </div>
               <p className="text-[9px] text-slate-400 font-bold uppercase italic">Expira en: 14:59 minutos</p>
            </div>
          </div>
        )}
        ...
        <section className="space-y-6">
          <div className="flex items-center justify-between">
             <h3 className="text-sm font-black uppercase tracking-[0.3em] text-slate-500">Asignaciones Programadas</h3>
             <span className="bg-slate-800 px-4 py-1.5 rounded-full text-[9px] font-black uppercase tracking-widest">{tasks.length} Tareas Activas</span>
          </div>
                  ...
                  <div className="space-y-1">
                    <div className="flex items-center space-x-3">
                       ...
                       <span className={`text-[8px] font-black px-2 py-0.5 rounded-sm uppercase ${task.type === 'installation' ? 'bg-blue-500/20 text-blue-400' : 'bg-amber-500/20 text-amber-400'}`}>
                          {task.type === 'installation' ? 'INSTALACIÓN' : task.type === 'repair' ? 'REPARACIÓN' : 'MANTENIMIENTO'}
                       </span>
                    </div>
        ...
        <section className="bg-blue-600/10 border border-blue-500/20 p-10 rounded-[2.5rem] flex items-center justify-between overflow-hidden relative group">
           ...
           <div className="space-y-4 relative z-10">
              <h4 className="text-xl font-black uppercase tracking-tighter italic">Recordatorio de Protocolo de Seguridad</h4>
              <p className="text-xs text-blue-200/60 max-w-md uppercase tracking-widest font-bold">Verificar siempre la identidad del cliente antes de iniciar cualquier instalación física. Reportar toda inconsistencia vía interfaz de Alerta de Emergencia.</p>
           </div>
      ...
      <footer className="p-8 text-center text-[9px] font-black text-slate-600 uppercase tracking-[0.5em]">
         Fibex Telecom • Sistema de Gestión de Agentes de Campo • v5.0.0 Master
      </footer>
    </div>
  );
};
